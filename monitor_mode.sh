...
# Script to set $interface to monitor mode and confirm success
...
if [ $? -ne 0 ]; then
    echo "Error: Failed set $interface to monitor mode"
    exit 1
fi
...
echo "Success: $interface running in monitor mode"
exit 0